#ifndef __COMMON_FIXED_VECTOR_H__
#define __COMMON_FIXED_VECTOR_H__
...
private:
    ...
    bool Move(T* begin, T* end) {
        ...
        for (T* it = begin; it != end; ++it) {
            PlaceAt(&items_[it - begin], std::move(*it));
        }
        size_ = count;
        return true;
    }
...
bool FixedVector<T, N>::Insert(const T* pos, const T& item) {
    ...
    PlaceAt(&items_[size_++], item);
    std::rotate(
        const_cast<T*>(pos), items_ + size_ - 1, items_ + size_);
    return true;
}
...
#endif  // __COMMON_FIXED_VECTOR_H__
